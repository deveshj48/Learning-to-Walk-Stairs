...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        self,
        # xml_file="custom_ant/ant.xml",
        # xml_file="C:/Users/deves/OneDrive/Desktop/sac_ant/custom_ant/scene.xml",
        xml_file = os.path.abspath("./custom_ant/scene.xml"),
        ...
        healthy_z_range=(0.2, 4),
        # healthy_z_range = 0.2, # lower bound on z position - no upper bound b=needed since stairs/
    ...
    def is_healthy(self):
        ...
        max_z = self.data.body("target").xpos.copy()[2] + 2 # get z position of target , and add onto that    
    ...
    #     target_position = self.data.body("target").xpos[:2].copy() # get target position - target is defined in .xml file         
    #     distance_to_target = np.linalg.norm(xy_position_after - target_position)
    #     # print(distance_to_target)
    #     target_reward = -distance_to_target # negate so rewards get smaller as we get further apart
    #     # target_reward  = (1 / (1+distance_to_target) ) - 1
    #     # Sum up the rewards
    #     rewards = forward_reward + healthy_reward + target_reward
    ...
    def step(self, action):
        ...
        target_position = self.data.body("target").xpos[:2].copy() # get target position - target is defined in .xml file         
        distance_to_target_before = np.linalg.norm(xy_position_before - target_position)
        distance_to_target_after= np.linalg.norm(xy_position_after - target_position)
        # print(distance_to_target)
        # target_reward = -distance_to_target # negate so rewards get smaller as we get further apart
        # target_reward  = (1 / (1+distance_to_target) ) - 1
        # Sum up the rewards
        ...
        rewards = 0
        if distance_to_target_after > distance_to_target_before:
            rewards -= 1
        # else:
        #     rewards -= 1
        # rewards += forward_reward + healthy_reward + target_reward
